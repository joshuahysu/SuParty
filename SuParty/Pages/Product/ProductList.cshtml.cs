...
namespace SuParty.Pages.Product
{
    public class ProductListModel : PageModel
    {
        ...
        public ProductListModel(ApplicationDbContext dbContext, IMemoryCache cache)
        {
        ...
        public async Task<IActionResult> OnGet()
        {
            //要有快取避免天天搜尋
            // 嘗試從快取中取得資料
            if (!_cache.TryGetValue("ProductListModel", out IQueryable<ProductData> data))
            {
                // 如果快取不存在，則取資料
                data = _dbContext.ProductDatas;
                productList = data.ToList();
                // 設置快取（持續 5 分鐘）
                _cache.Set("ProductListModel", data, TimeSpan.FromMinutes(5));
            }
